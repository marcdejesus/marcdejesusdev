...
interface UpdateCardProps { // Renamed from BlogPostCardProps
  post: UpdatePost; // Renamed type
  index: number;
}

export function UpdateCard({ post, index }: UpdateCardProps) { // Renamed component and props
  const cardVariants = {
      ...
      <Card className="flex flex-col h-full overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
        <div className="relative w-full h-56">
          <Image
            ...
            data-ai-hint={post.dataAiHint || 'update image feature'} // Updated data-ai-hint
          />
        </div>
        <CardHeader>
        ...
        <CardFooter className="pt-4">
          <Button asChild variant="default" size="sm" className="w-full">
            <Link href={`/updates/${post.slug}`}> {/* Changed href from /blog to /updates */}
